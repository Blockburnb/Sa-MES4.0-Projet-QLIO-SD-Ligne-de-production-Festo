...
# Note: This frontend is a near-copy of the original maquette UI. It attempts to fetch from the backend at BACKEND_URL for /orders but falls back to random/simulated data when the backend is not reachable.
# To run: pip install -r requirements.txt then run `streamlit run frontend/maquette.py`.
